...
TOOL_NAME = "BIASCOPE"
TOOL_VERSION = "0.1.0"
RULES = [{'id': 'BC-DEMO-001', 'severity': 'high', 'weight': 2.5, 'title': 'DEMOGRAPHIC_DISPARITY', 'when': {'axis': 'demographic', 'disparity': '^(?:0\\.[3-9]|[1-9])'}, 'remediation': 'Investigate disparate impact; retrain or add post-processing fairness.'}, {'id': 'BC-OCC-001', 'severity': 'medium', 'weight': 2.0, 'title': 'OCCUPATIONAL_STEREOTYPE', 'when': {'axis': 'occupational', 'stereotyped': True}, 'remediation': 'Audit training data for stereotype reinforcement.'}]
        ...
        for rule in RULES:
            if _match(item, rule):
                result.add(Finding(
                    ...
                    location=str(target),
                    remediation=rule.get("remediation",""), category="bias",
                    metadata=item,
                ))
