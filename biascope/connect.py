"""Native cognis-connect emit for biascope — forward findings to any platform.

Maps biascope's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    biascope ... --format json | biascope-emit --to stix
    biascope-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        out['title'] = rec.get('probe_id', '') + ': ' + rec.get('group', '')
        out['severity'] = rec.get('category', '').lower()
        out['type'] = rec.get('kind', '')
        out['description'] = rec.get('detail', '')
        out['tags'] = [rec.get('category', ''), rec.get('group', '')]
